...
describe("Path of Exile - Trade", function () {
    ...
    it("#search(league, query) - should return search results", async () => {
        ...
        await expect(API.search("Standard", query)).to.be.fulfilled;
    });
    ...
    it("#exchange(league, query) - should return exchange results", async () => {
        ...
        await expect(API.exchange("Standard", query)).to.be.fulfilled;
    });

    it("#getFromHashes(hashes) - should return search results", async () => {
    ...
    it("#getIgnoredAccounts() - should return list of ignored accounts", async () => {
        await expect(API.getIgnoredAccounts()).to.be.fulfilled;
    });
});
